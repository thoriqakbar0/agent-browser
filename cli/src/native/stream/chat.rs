...
- To use a different browser engine: add `--engine <engine>` (e.g. `agent-browser --session chrome-session --engine chrome open https://example.com`). Supported engines: camofox (default), chrome, lightpanda.
